...
<template>
  <div class="container flex flex-col gap-6 p-4">
    <UFormField
      ...
      :error="validationErrors.animalRace"
    >
      <select
        id="input-race"
        v-model="state.step1.animalRace"
        class="w-full p-2 border rounded bg-white text-slate-900 border-slate-300"
      >
        <option value="">
          Selecione a raça
        </option>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
      ...
      >
        <UInput
          id="input-age"
          v-model.number="state.step1.age"
          type="number"
          placeholder="Ex: 24"
          class="w-full"
        />
      ...
      <UFormField
        label="Peso vivo (kg)"
        :error="validationErrors.liveWeight"
      >
        <UInput
          id="input-weight"
          type="number"
          :model-value="state.step1.liveWeight"
          placeholder="Ex: 450"
          class="w-full"
          @update:model-value="handleWeightInput"
        />
      </UFormField>
    </div>
    ...
    <UFormField
      ...
      :error="validationErrors.dietObjective"
    >
      <div class="objective-grid grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div
          v-for="obj in dietObjectives"
    ...
    <CmsSelector :peso-vivo="Number(state.step1.liveWeight)" />
  </div>
</template>
